...
public abstract class CustomSingletonModel : SingletonModel, ICustomModel
{
	...
	private static readonly Type? RunHookSubDelegate = Type.GetType("MegaCrit.Sts2.Core.Modding.RunHookSubscriptionDelegate, sts2");

	private static readonly Type? CombatHookSubDelegate = Type.GetType("MegaCrit.Sts2.Core.Modding.CombatHookSubscriptionDelegate, sts2");
	...
	public CustomSingletonModel(bool receiveCombatHooks, bool receiveRunHooks)
	{
		...
		{
			BaseLibMain.Logger.Warn("CustomSingleton " + ((object)this).GetType().FullName + " created; not supported on current game branch", 1);
			return;
		}
		if (receiveRunHooks)
		{
			SubscribeRunState.Invoke(null, new object[2]
			{
				((AbstractModel)this).Id.Entry,
				Delegate.CreateDelegate(RunHookSubDelegate, this, typeof(CustomSingletonModel).GetMethod("RunSubModels"))
			});
		}
		if (receiveCombatHooks)
		{
			SubscribeCombat.Invoke(null, new object[2]
			{
				((AbstractModel)this).Id.Entry,
				Delegate.CreateDelegate(CombatHookSubDelegate, this, typeof(CustomSingletonModel).GetMethod("CombatSubModels"))
			});
		}
	}
